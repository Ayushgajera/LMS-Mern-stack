...
import { FiHome, FiBook, FiMenu, FiX } from 'react-icons/fi';
...
function Sidebar() {
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <aside className="hidden lg:flex h-screen w-64 flex-col fixed left-0 top-0 bottom-0 bg-white border-r border-gray-200 z-30">

        {/* Navigation */}
        <nav className="flex-1 p-4 space-y-2 overflow-y-auto mt-18">
          {menuItems.map((item) => (
            <Link
              key={item.path}
              to={item.path}
        ...
        {isMobileMenuOpen && (
          <>
            ...
            <motion.div
              ...
              className="lg:hidden fixed inset-0 z-40 bg-gray-600/50 backdrop-blur-sm"
            />
